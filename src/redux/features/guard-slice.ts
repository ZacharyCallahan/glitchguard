...
export const guard = createSlice({
  ...
  reducers: {
    ...
    deleteGuard: (state, action: PayloadAction<TempGuardPayloadForDelete>) => {
    ...
    deleteBoard: (state, action: PayloadAction<TempBoardPayloadForDelete>) => {
    ...
    deleteList: (state, action: PayloadAction<TempListPayloadForDelete>) => {
    ...
    deleteBug: (state, action: PayloadAction<TempBugPayloadForDelete>) => {
      ...
      if (list) {
        list.bugs = list.bugs.filter((bug) => bug.id !== action.payload.bugId);
      }
    },
    ...
    editList: (state, action: PayloadAction<ListPayload>) => {
      ...
      // Find the list that matches the list that was passed in.
      const list = board?.lists.find((list) => list.id === action.payload.id);
